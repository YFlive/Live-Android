...
//        findViewById(R.id.btn_re_record).setOnClickListener(this);
        ...
        //按钮动画相关
        mVideoRecordBtnView = (VideoRecordBtnView) findViewById(com.yuanfen.video.R.id.record_btn_view);
        mRecordView = findViewById(com.yuanfen.video.R.id.record_view);
        mUnRecordDrawable = ContextCompat.getDrawable(mContext, com.yuanfen.video.R.drawable.bg_btn_record_1);
        mRecordDrawable = ContextCompat.getDrawable(mContext, com.yuanfen.video.R.drawable.bg_btn_record_2);
    ...
    public void onRecordEvent(int event, Bundle bundle) {
        if (event == TXRecordCommon.EVT_CAMERA_CANNOT_USE) {
            ToastUtil.show(com.yuanfen.video.R.string.video_record_camera_failed);
        } else if (event == TXRecordCommon.EVT_MIC_CANNOT_USE) {
            ToastUtil.show(com.yuanfen.video.R.string.video_record_audio_failed);
        }
    }
        ...
        if (mRecorder != null) {
            ...
            if (result != TXRecordCommon.START_RECORD_OK) {
                if (result == TXRecordCommon.START_RECORD_ERR_NOT_INIT) {
                    ToastUtil.show(com.yuanfen.video.R.string.video_record_tip_1);
                } else if (result == TXRecordCommon.START_RECORD_ERR_IS_IN_RECORDING) {
                    ToastUtil.show(com.yuanfen.video.R.string.video_record_tip_2);
                } else if (result == TXRecordCommon.START_RECORD_ERR_VIDEO_PATH_IS_EMPTY) {
                    ToastUtil.show(com.yuanfen.video.R.string.video_record_tip_3);
                } else if (result == TXRecordCommon.START_RECORD_ERR_API_IS_LOWER_THAN_18) {
                    ToastUtil.show(com.yuanfen.video.R.string.video_record_tip_4);
                } else if (result == TXRecordCommon.START_RECORD_ERR_LICENCE_VERIFICATION_FAILED) {
                    ToastUtil.show(com.yuanfen.video.R.string.video_record_tip_5);
                }
                return;
            }
        }
