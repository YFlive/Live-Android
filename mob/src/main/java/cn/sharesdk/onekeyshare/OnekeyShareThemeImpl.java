...
import static cn.sharesdk.onekeyshare.OnekeyShare.SHARESDK_TAG;
...
public abstract class OnekeyShareThemeImpl implements PlatformActionListener, Callback {
	...
	final void shareSilently(Platform platform) {
		if (formateShareData(platform)) {
			...
			if (sp != null) {
				if (null != shareParamsMap) {
					Boolean isDispear = shareParamsMap.containsKey("disappearsharetoast") ? (Boolean) shareParamsMap.get("disappearsharetoast") : false;
					if (!isDispear) {
						toast("ssdk_oks_sharing");
					} else {
//						SSDKLog.getInstance().d(SHARESDK_TAG, "isDispear is false");
					}
				}

				if (customizeCallback != null) {
					customizeCallback.onShare(platform, sp);
//					SSDKLog.getInstance().d(SHARESDK_TAG, "customizeCallback.onShare(platform, sp)");
				}
				if (disableSSO) {
					platform.SSOSetting(disableSSO);
//					SSDKLog.getInstance().d(SHARESDK_TAG, "platform.SSOSetting(disableSSO)");
				}
